package au.com.energyaustralia.logging;
 ...
 *DailyRollingFileAppender} so most of the
...
public class CustodianDailyRollingFileAppender extends FileAppender
{
	...
	private String compressBackups = "true";
	...
	protected void cleanupAndRollOver() throws IOException
	{
		...
		{
			LogLog.error("max days must be set to an int");
		}
		...
		if (file.getParentFile().exists())
		{
			...
			for (int i = 0; i < files.length; i++)
			{
				...
				{
					LogLog.error("This isn't a file we should touch (it isn't named correctly )" + pe);
				}
			}
		}
		rollOver();
